...
void _fixOfferingTypeForSelectedService() {
  ...
  if (pickedVideo != null) {
    setState(() {
      _selectedReelVideos.add(File(pickedVideo.path));
    });
  }
}

void _removeReelVideo(int index) {
  setState(() {
    _selectedReelVideos.removeAt(index);
  });
}
  ...
  children: [
    if (_selectedReelVideos.isEmpty)
      GestureDetector(
        onTap: _pickReelVideo,
        child: Container(
          height: 112,
          decoration: BoxDecoration(
      ...
      Column(
        children: _selectedReelVideos.asMap().entries.map((entry) {
          final index = entry.key;
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                IconButton(
                  onPressed: () => _removeReelVideo(index),
                  icon: const Icon(
                    Icons.close_rounded,
                    color: maroon,
                  ),
